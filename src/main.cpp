...
uint256 hashGenesisBlock("0x8b8b9ca90798798c58fc62c599f7b790450549ae3dc340f6148973eeb1686deb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fanchuancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Fanchuancoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	{
		nSubsidy = 4 * COIN;
	}else if (nHeight < 38880)
	{
		nSubsidy = 7 * COIN;
	}else if (nHeight < 43200)
	{
		nSubsidy = 10 * COIN;
	}else if (nHeight < 84752)
	{
		nSubsidy = 7 * COIN;
	}else if (nHeight < 92080)
	{
		nSubsidy = 4 * COIN;
...
static const int64 nTargetTimespan = 60 * 60;	// Fanchuancoin: 1 hour
static const int64 nTargetSpacing = 60;						// Fanchuancoin: 30 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 60 * 60; // 60 Minutes
static const int64 nTargetSpacingRe = 30; // Fanchuancoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	// Fanchuancoin: This fixes an issue where a 51% attack can change difficulty at will.
		...
		const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
	...
	// fMiner is true when called from the internal fanchuancoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
	{
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "Fanchuancoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	if (fTestNet)
	{
		...
		hashGenesisBlock = uint256("0x8b8b9ca90798798c58fc62c599f7b790450549ae3dc340f6148973eeb1686deb");
	}
	...
	if (mapBlockIndex.empty())
	{
		...
		// Genesis block
		const char* pszTimestamp = "Fan chuan coin";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		...
		block.nVersion = 1;
		block.nTime    = 1395317915;
		block.nBits    = 0x1e0ffff0;
		block.nNonce   = 1867770 ;
		...
		assert(block.hashMerkleRoot == uint256("0xe282f6a851b843f829727a7f5fee1b8063ecb330b1b1dad7e4a96e674730f338"));
...
{
	return false;
	//commenting out this code until we properly implement FCC alerts.
	/*
	   if (!CheckSignature())
	   return false;
	   if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fanchuancoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
	...
	int64 nFees = 0;
	{
		...
		while (!mapPriority.empty())
		{
			...
			// Fanchuancoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
